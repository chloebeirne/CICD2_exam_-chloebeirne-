...
def test_create_author(client):
    r = client.post("api/authors", json = {"name": "Chloe", "email": "chloebeirne@atu.ie", "year": = 2003, "author_id": C123456789})
    assert r.status_code == 201

def test_put_author(client):
    r = client.post("api/authors", json = {"name": "Ciaran", "email": "ciaranb@atu.ie", "year": = 2004, "author_id": C987654321})
    assert r.status_code == 201
    author_id = r.json()["id"]